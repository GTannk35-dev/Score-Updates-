...
import type { Game, PlayerStat, ScoresPayload } from "@/lib/types";
...
function BroadcastStage({ children }: { children: React.ReactNode }) {
  ...
  return <div className="stage">
    {scale > 0 ? <div className="stage__frame" style={{ width: STAGE_W, height: STAGE_H, transform: `translate(-50%, -50%) scale(${scale})` }}>
      {children}
    </div> : null}
  </div>;
}

function PlayerStatsTake({ player }: { player: PlayerStat }) {
  return <div className="player-stats-take" role="status" aria-live="polite">
    <div className="player-stats-take__wash" />
    <div className="player-stats-take__inner">
      <span className="player-stats-take__eyebrow">BIG 9 PLAYER SPOTLIGHT</span>
      <Brand />
      <span className="player-stats-take__rule" />
      <div className="player-stats-take__identity"><strong>{player.name}</strong><span>{player.team}{player.position ? ` · ${player.position}` : ""}</span></div>
      <div className="player-stats-take__stats">{Object.entries(player.stats).map(([label, value]) => <div key={label}><b>{value}</b><span>{label.replace(/[-_]/g, " ")}</span></div>)}</div>
    </div>
  </div>;
}
...
export function BroadcastBoard() {
  ...
  const selectedPlayer = games.flatMap((entry) => entry.playerStats ?? []).find((player) => player.id === playerId);
  ...
  useEffect(() => subscribeToState((state) => {
    ...
    if (state.playerId !== undefined) setPlayerId(state.playerId || null);
    if (state.showPlayerStats !== undefined) setShowPlayerStats(state.showPlayerStats);
  ...
  if (awaitingFeed) return <BroadcastStage><main className="broadcast"><div className="broadcast__glow" /><div className="broadcast__grain" />{stinger && <ScoreUpdateStinger key={stinger.key} sportLabel={sportLabel} kind={stinger.kind} custom={stinger.custom} hold={stinger.hold} />}{showPlayerStats && selectedPlayer && <PlayerStatsTake player={selectedPlayer} />}<header className="broadcast-header"><a className="brand-link" href="/operator" aria-label="LMR Media — operator controls"><Brand /></a></header><div className="empty-state"><span className="empty-state__eyebrow">LMR MEDIA / BIG 9 CONFERENCE</span><h1>Standing by</h1><p>Waiting for the operator console…</p></div></main></BroadcastStage>;
  ...
  return <BroadcastStage><main className="broadcast">
    ...
    {showPlayerStats && selectedPlayer && <PlayerStatsTake player={selectedPlayer} />}
...
export function OperatorPanel() {
  ...
  useEffect(() => {
    ...
    if (snapshot.playerId !== undefined) { setSelectedPlayerId(snapshot.playerId); }
  ...
  const resetBoard = () => {
    setSport(DEFAULT_SPORT); setDates([]); setSchools([]); setGameIds([]); setIndex(0); setPaused(false); setCustomHeadline(""); setArmed("score"); setSelectedPlayerId("");
    publishState({ sport: DEFAULT_SPORT, dates: [], paused: false, schools: [], gameIds: [], stepTo: 0, playerId: "", showPlayerStats: false });
  };
  const playerOptions = games.flatMap((entry) => (entry.playerStats ?? []).map((player) => ({ ...player, game: `${entry.away.shortName} @ ${entry.home.shortName}` })));
  const showSelectedPlayer = () => { if (!selectedPlayerId) return; publishState({ sport, dates, paused, schools, gameIds, stepTo: index, playerId: selectedPlayerId, showPlayerStats: true }); setUpdateSent("Player stats sent to air"); window.setTimeout(() => setUpdateSent(null), 1800); };
  const hidePlayerStats = () => { publishState({ sport, dates, paused, schools, gameIds, stepTo: index, playerId: selectedPlayerId, showPlayerStats: false }); };
